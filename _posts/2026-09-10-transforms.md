...
- The **fractional Fourier transform**, which is a power of the DFT rather than a new kernel. The DFT matrix $F$ is unitary and $F^2$ is the reflection $f(x) \mapsto f(-x)$, so $F^4 = I$ and the eigenvalues of $F$ are among $\pm 1, \pm i$; $F^\alpha$ is defined on that eigenbasis. Their multiplicities are $m+1, m, m, m-1$ for $N = 4m$, and they are fixed by the trace of $F$, which is a quadratic [Gauss sum](https://en.wikipedia.org/wiki/Quadratic_Gauss_sum) that Gauss evaluated without the matrix.[^dftspectrum]
...
[^dftspectrum]: CST 2018, Proposition 4.1.2 ($F^4 = I$) and §4.3–4.4, where the trace of $F$, a Gauss sum, yields quadratic reciprocity. AT, Theorem 1.1.2′, p. 857 (multiplicities, from Schur's $F^2$ = reflection, p. 856), Theorem 1.1.2, p. 855 (Gauss's trace).
