...
- The **DCT**, which is the DFT on $2N$ points folded by the reflection $n \mapsto -n$.[^dct]
...
- The **fractional Fourier transform**, which is a power of the DFT rather than a new kernel. The DFT matrix $F$ is unitary and $F^2$ is the reflection $f(x) \mapsto f(-x)$, so $F^4 = I$ and the eigenvalues of $F$ are among $\pm 1, \pm i$; $F^\alpha$ is defined on that eigenbasis.[^dftspectrum]
...
**Laplace, $Z$, chirp $Z$: characters off the unitary axis.** $e^{-st}$ with $\Re s > 0$ is a character of $(\mathbb{R}, +)$ continued off the imaginary axis, which is why it decays. The **Laplace transform** is the Fourier transform on the half-line with that continuation built in, easier to handle because the kernel decays and the result is analytic in $s$, and it pays for that with Plancherel: there is no isometry.[^laplace] The **$Z$-transform** makes the same bargain on $\mathbb{Z}$; the **chirp $Z$-transform** evaluates it along a spiral.

**Radon: an [intertwiner](https://en.wikipedia.org/wiki/Equivariant_map), not a change of basis.** The **Radon transform** carries functions on points to functions on lines, and the rigid motions act on both; it intertwines two representations of one group. There is a finite model in which everything is exact. Let $S_n$ act on the $k$-subsets and the $j$-subsets of $\{1, \dots, n\}$, and let $D$ send a function on $k$-subsets to the function on $j$-subsets obtained by summing over the $k$-subsets containing each one. That is a Radon transform, and it has an explicit inverse. In general the intertwiners from $L(G/K)$ to $L(G/H)$ are the functions on $G$ invariant under $K$ on the left and $H$ on the right, so a Radon transform is an incidence relation read as such a function.[^radon]
...
[^dates]: Clairaut 1754, Gauss 1805 and Cooley–Tukey 1965: Terras, pp. xv–xvi and 30–31, after Heideman, Johnson and Burrus (1984). Laplace in the 1780s, Fourier 1822: Debnath–Bhatta, §1.1. Further: Mackey, §§3–8, for the three origins of harmonic analysis.
...
[^halmos]: Halmos, §58 (Jordan form), §79 (spectral theorem), §84 Exercise 6 (simultaneous diagonalization), §81 (real orthogonal transformations as plane rotations).

[^mackey]: Mackey, §1 for the method, §2 (pp. 547–548) for translation and differentiation becoming multiplication, §9 (pp. 570–575) for the potential and heat-kernel formulas and for the sphere. Further: §23, his summary.
...
[^dftspectrum]: CST 2018, Proposition 4.1.2 ($F^4 = I$) and §4.3–4.4, where the trace of $F$, a Gauss sum, yields quadratic reciprocity.
...
[^wedderburn]: Steinberg, Theorem 5.5.6; CST 2008, Theorem 9.5.4, for $(G \times G, \Delta G)$; Kuperberg, §8.2, for row and column spaces.

[^diaconis]: Diaconis, Chapter 3B, Lemma 1, Theorems 2 and 5; conjugacy classes as canonical forms, p. 14. Cayley graphs: Steinberg, Theorem 5.4.10 and Exercise 5.12. Further: Diaconis, Chapter 3.

[^laplace]: Debnath–Bhatta, §1.1, p. 2; Mackey, pp. 573–574.

[^fft]: CST 2018, §10.7 and Exercise 10.7.3 (Diaconis–Rockmore, with Cooley–Tukey as the case $K = \mathbb{Z}_m$); §5.4 for Rader. Further: CST 2018, Chapter 5.

[^hsp]: Nielsen–Chuang, §5.4.3. Kuperberg, §§3–4 for the algorithm and the $V_k$, §8.2 for the character measurement, §9 for the general sieve.

[^heisenberg]: CST 2018, Theorem 12.4.2: the DFT intertwines $\pi'$ with $\pi' \circ J$, and $J$ has order four. Further: §12.5, where the FFT is derived the same way, after Auslander and Tolimieri.
