...
1. Check ArchLinux [website status](https://status.archlinux.org/) since they are prone to [DDoS attacks](https://lists.archlinux.org/archives/list/arch-general@lists.archlinux.org/thread/EU4NXRX6DDJAACOWIRZNU4S5KVXEUI72/). Also, the services may send an initial connection reset due to the TCP SYN authentication performed by the hosting provider, but subsequent requests should work as expected.
    ...
    2. In the case of downtime for `aur.archlinux.org`:
        1. Packages: A mirror of AUR packages is maintained on GitHub. You can retrieve a package using: 
            ...
            ```` 
2. 
3. Download the latest `archlinux-YYYY.MM.DD-x86_64.iso`, `sha256sums.txt`, and `archlinux-YYYY.MM.DD-x86_64.iso.sig` from the [nearest mirror](https://archlinux.org/download/#download-mirrors) like `mirror.arizona.edu`.
4. Verify the integrity and authenticity of your ISO image using Linux (if Windows, use WSL). 
...
5. Create a bootable USB drive using [Ventoy](https://www.ventoy.net/en/download.html); remember to verify the checksum of Ventoy itself.
6. In UEFI, disable [secure boot](https://wiki.archlinux.org/title/Unified_Extensible_Firmware_Interface/Secure_Boot).
...
9. Use [`pacstrap`](https://man.archlinux.org/man/pacstrap.8) to create a new system installation from scratch.
    ...
    ````
    Moreover, I was unable to later run `pacman -S networkmanager` and `systemctl enable NetoworkManager` under `arch-chroot`.
...
11. Switch to the new system's environment
    ...
    4.  Create a new `initramfs` (*init*ial *RAM* *f*ile *s*ystem)
        ...
        2. Install [kernel firmware](https://wiki.archlinux.org/title/Linux_firmware) (`linux-firmware`) along with additional firmware for [Intel CPU](https://wiki.archlinux.org/title/Microcode) (`intel-ucode`), [Intel iGPU](https://wiki.archlinux.org/title/Intel_graphics) (`mesa`, `vulkan-intel`), [NVIDIA dGPU](https://wiki.archlinux.org/title/NVIDIA) (`nvidia`, `nvidia-utils`), [Dolby Atmos](https://gitlab.freedesktop.org/pipewire/pipewire/-/wikis/Filter-Chain#virtual-surround) (`pipewire`, `pipewire-alsa`, `pipewire-pulse`, `pipewire-jack`, `wireplumber`), and userspace utilities for the [Btrfs file system](https://wiki.archlinux.org/title/File_systems) (`btrfs-progs`). 
           ````
           # pacman -S linux-firmware intel-ucode mesa vulkan-intel nvidia nvidia-utils pipewire pipewire-{alsa,pulse,jack} wireplumber btrfs-progs
           ````
...
4. Mimic [Lenovo Vantage](https://wiki.archlinux.org/title/Laptop/Lenovo) to manage UEFI updates and battery control using `fwupd` and `tlp`.
   ...
   # tlp-stat -b
   ````
   We can configure `tlp` by editing [battery care settings](https://linrunner.de/tlp/settings/bc-vendors.html) in `/etc/tlp.conf`. It is an optional dependecy of [KDE's `powedevil`](https://archlinux.org/packages/extra/x86_64/powerdevil/).
...
7. Install KDE with [Wayland support](https://archlinux.org/news/plasma-640-will-need-manual-intervention-if-you-are-on-x11/) following advice from the [KDE documentation](https://community.kde.org/Distributions/Packaging_Recommendations) and Arch dependency tree of [`plasma-desktop`](https://archlinux.org/packages/extra/x86_64/plasma-desktop/). We have consciously excluded `discover` from KDE because it can lead to partial upgrades [which are unsupported in Arch](https://wiki.archlinux.org/title/System_maintenance#Partial_upgrades_are_unsupported).
    1. base: `plasma-{desktop,pa,nm,systemmonitor,firewall}`, `kscreen`, `bluedevil`, `powerdevil`
...
4. Install [essential software](https://pkgstats.archlinux.de/fun) using the [package manager](https://wiki.archlinux.org/title/Pacman). 
    ...
    14. Hybrid GPUs ([NVIDIA Optimus](https://wiki.archlinux.org/title/NVIDIA_Optimus)): `nvidia-settings`
    15. IDE: `emacs-wayland`
...
Most of these tasks can be done usin approprite pacmanhooks: https://www.reddit.com/r/archlinux/comments/dsnu81/hear_ye_archers_share_your_pacman_hooks/
...
5. To get a comprehensive snapshot of the Linux system use [`inxi`](https://man.archlinux.org/man/extra/inxi/inxi.1.en).
   ```` 
   # pacman -S inxi
...
7. To view a list of snapshots under `root` configurations.
   ...
   # snapper -c root list
   ````
8. If GUI fails then can use LiveUSB [arch-chroot](https://wiki.archlinux.org/title/Chroot) or Desktop environment's [tty](https://wiki.archlinux.org/title/Keyboard_shortcuts#Xorg_and_Wayland).
