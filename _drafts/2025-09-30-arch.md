...
9. To restore `/` using one of snapper's snapshots, we need access to OverlayFS (either via LiveUSB or `grub-btrfs`) and [follow these steps](https://wiki.archlinux.org/title/Snapper#Restoring_/_to_its_previous_snapshot ).
    ````
    # fdisk -l
    # mount /dev/root_partition /mnt
    # lsblk --fs
    ...
    # reboot
    ````
10. In theory,  we can use the [`grub-btrfs` daemon](https://github.com/Antynea/grub-btrfs?tab=readme-ov-file#-automatically-update-grub-upon-snapshot-creation-or-deletion) to automatically update GRUB upon snapshot creation or deletion.
   ````
   ...
   # mkinitcpio -P
   ````
   However, when booting from the snapshots I always got the following error:
    ````
    ********************* WARNING *********************
    ...
    ***************************************************
    ...
    ````
    Therefore, I decided to stick with LiveUSB.
