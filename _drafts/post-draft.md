...
9. Install the latest version of Ubuntu Terminal (304 MB downloaded) which allows new tabs via `Ctrl`+`Shift`+`T`: `sudo apt -t testing install gnome-terminal` (It also uses GTK interface, so if you might consider installing vim-gtk3 as well)
