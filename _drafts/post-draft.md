...
3. Open terminal and use Vim to edit, if needed, the "cros.list" and "sources.list" so that they have the following content (make sure it is "http" and not "https"):  
   ````
   asus@penguin:~$ cat /etc/apt/sources.list.d/cros.list
   deb http://storage.googleapis.com/cros-packages/83 buster main
   
   asus@penguin:~$ cat /etc/apt/sources.list
...
6. Remove the installed version of Vim (we will do a fresh install of the latest GUI version): `sudo apt remove vim vim-common vim-tiny vim-runtime` and then `sudo apt autoremove`
7. Install the latest version of Vim: `sudo apt-get -t testing install vim`. You can also try the GUI version like GTK3, but scrren resolution is all messed up with Crostini in my chromebook do I will stick with [terminal](https://www.reddit.com/r/Crostini/comments/b3mff0/opening_a_new_tab_in_the_terminal/).
8. Install the latest version of Texlive: `sudo apt-get -t testing install texlive`
