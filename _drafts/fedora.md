...
Select Xfce Fedora without any extra software programs, we will manually install the ones we want. To bypass creating root password in netinstall mode, just create a user account and mark it as administrator. After installing, the first think we will do is set-up drop-down terminal following [this guide](https://averagelinuxuser.com/xfce-drop-down-terminal/).
