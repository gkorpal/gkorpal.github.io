...
Command: `M-x rust-run`
Command: `M-x rust-compile`

That "tilde clutter" is a classic Emacs behavior. Those files (ending in `~`) are backup files that Emacs creates automatically every time you save a file.

You can tell Emacs to keep making backups but store them all in one specific folder (like ~/.emacs.d/backups) instead of scattering them next to your source code.

Add this to your Emacs configuration file (usually ~/.emacs or ~/.emacs.d/init.el):
Code snippet
...
This keeps your repository clean while still giving you the safety net of backup files.
